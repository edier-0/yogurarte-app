...
export const preparationIngredientSchema = z.object({
  rawMaterialId: z.coerce.number().int().positive('ID de materia prima inválido'),
...
export const createPreparationSchema = z.object({
  ...
  ingredients: z.array(preparationIngredientSchema).min(1, 'Debe incluir al menos un ingrediente o insumo a la receta'),
});

export type PreparationIngredientInput = z.infer<typeof preparationIngredientSchema>;
